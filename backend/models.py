...
class Level(Base):
    ...
    category: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # 'physical' | 'social' | 'time'
...
class Run(Base):
    ...
    started_at = mapped_column(TIMESTAMP(timezone=True), server_default=func.now())
...
class RunStep(Base):
    ...
    level_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("levels.id", ondelete="CASCADE"))
